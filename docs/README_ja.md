...
of daemon enable                     # macOS ログイン項目として登録
...
# 既存のボールトで再実行して修復/アップグレード（冪等性 — 上書きされません）
...
ボールト間で `.obsidian/` プラグイン、テーマ、スニペットを同期します。
...
```bash
obsidian-forge graph health                 # 統計と健康メトリクスを表示
obsidian-forge graph orphans [--auto-link]  # 孤立ノートを一覧表示（または AI で自動リンク）
obsidian-forge graph extract [--no-ai]      # リンクと関係性を抽出
obsidian-forge graph tags [--dry-run]       # タグの正規化とクラスタリング
obsidian-forge graph strengthen             # フルパイプラインを実行

# 既存のエイリアス（フルパイプラインを実行）
...
obsidian-forge status             [--vault <name>]   # 設定と AI ステータスを表示
obsidian-forge doctor             [--vault <name>]   # ボールトの健康状態を診断
```

### バックグラウンドデーモン（macOS LaunchAgent）

```bash
obsidian-forge daemon enable     # plist 書き込み + ブートストラップ（ログイン項目）
obsidian-forge daemon disable    # ブートアウト + plist 削除
obsidian-forge daemon start
obsidian-forge daemon stop
obsidian-forge daemon restart
obsidian-forge daemon status     # PID、最後の終了コード、スケジュールされたボールトを表示
```
...
│   │   ├── scan.rs      ボールト全体のグラフスキャン
│   │   ├── tags.rs      コンセプトベースの自動タグ付け
│   │   ├── wikilinks.rs ウィキリンクの抽出と注入
│   │   ├── backlinks.rs バックリンクセクションの生成
│   │   ├── bridges.rs   ブリッジノートの作成
│   │   ├── relationships.rs 関連プロジェクトのリンク
│   │   ├── orphans.rs   孤立ノートの検出
│   │   ├── autotag.rs   自動タグ付けのオーケストレーション
│   │   └── health.rs    グラフの健康状態レポート
...
obsidian-forge は **[alcove](https://github.com/epicsagas/alcove)** の姉妹プロジェクトです。alcove は AI エージェントにプロジェクトドキュメントを提供する MCP サーバーです。これらは Cargo ワークスペースを共有し、個人の知識とプロジェクトインテリジェンスの間のループを完成させます：

- **obsidian-forge** = **大工 (The Forge)** (書き込み/プッシュ)。ボールトのメンテナンスを自動化し、ナレッジグラフを強化し、git に同期するバックグラウンドデーモンです。
- **alcove** = **図書館 (The Library)** (読み取り/プル)。コンテキストウィンドウを肥大化させることなく、AI エージェントにオンデマンドで検索可能なドキュメントアクセスを提供する MCP サーバーです。

```mermaid
graph LR
    A[Obsidian ボールト] -->|of daemon| B(obsidian-forge)
    B -->|of sync| C[Git リポジトリ]
    A -->|alcove promote| D[.alcove / docs]
    D -->|MCP ツール| E[AI エージェント]
    E -.->|参照| D
```

### Alcove との統合

`obsidian-forge` がナレッジグラフの構築と自動化に焦点を当てている一方で、[Alcove](https://github.com/epicsagas/alcove) はその知識が AI コーディングエージェントにとって実行可能であることを保証します。

#### 併用方法：

1.  **Obsidian で構築**: `obsidian-forge` を使用してボールトの健康状態を維持し、MOC を作成し、関連するコンセプトを自動的にリンクします。
2.  **プロジェクトドキュメントへの昇格**: ノート（例：アーキテクチャの決定や機能仕様）をプロジェクトで使用する準備ができたら、`alcove promote --source path/to/note.md` を実行します。
3.  **エージェントによる発見**: AI エージェント（Alcove MCP サーバーを使用）は、チャットにコピー＆ペーストすることなく、`search_project_docs` または `get_doc_file` を通じてそのノートを「発見」できるようになります。
4.  **ポリシー準拠**: Alcove の `validate_docs` を使用して、昇格させたノートがプロジェクトのドキュメント標準（`policy.toml` で定義）を満たしていることを確認します。
