...
**Генератор хранилищ Obsidian, демон автоматизации и набор инструментов для обслуживания**
...
`obsidian-forge` — это Rust CLI, которая создаёт, автоматизирует и поддерживает хранилища [Obsidian](https://obsidian.md). Работает как фоновый демон, отслеживая вашу папку входящих, проверяя целостность хранилища и синхронизируя с git — чтобы вы могли сосредоточиться на написании.

```
of init my-brain          # создать новое хранилище за секунды
of daemon enable         # зарегистрировать как элемент входа macOS
# → ваше хранилище теперь обрабатывается, проходит проверку здоровья и фиксируется автоматически
...
| 🛡️ | **Целостность хранилища** | Проверки тегов, проверка битых ссылок (с учётом код-блоков), нормализация frontmatter — всё с `--fix` |
| 📊 | **Здоровье графа** | Метрики заметок/ссылок/сирот/битых ссылок для вашего цикла линтинга |
| 📥 | **Обработка входящих** | Внедрение frontmatter, классификация ИИ, маршрутизация PARA |
| 🔄 | **Цикл синхронизации** | Проверка здоровья графа → автоматический git-коммит/пуш по таймеру |
| 🗂️ | **Несколько хранилищ** | Один демон управляет всеми хранилищами; флаги каждого хранилища в глобальной конфигурации |
...
obsidian-forge поставляется с 4 встроенными навыками агента, которые предоставляют ИИ-ассистентам контекстно-зависимые операции с хранилищем:

| Навык | Триггер |
|-------|---------|
| `vault-health` | Проверка здоровья хранилища, диагностика хранилища, статус хранилища |
| `vault-sync` | Синхронизация хранилища, проверка здоровья графа, фиксация изменений хранилища |
...
```bash
# 1. Создать новое хранилище (регистрирует его в глобальной конфигурации)
of init my-brain

# 2. Открыть в Obsidian → Файл → Открыть хранилище → my-brain

# 3. Установить фоновый демон
of daemon enable
...
Хранилища регистрируются автоматически командой `init` (безопасно запускать повторно в существующей директории).
Флаги каждого хранилища (`enabled`, `watch`) находятся в `~/.config/obsidian-forge/config.toml`:

```toml
[[vaults]]
name    = "my-brain"
path    = "/path/to/my-brain"
enabled = true    # включено в синхронизацию
watch   = true    # наблюдается демоном
```

### Целостность хранилища и операции с графом

```bash
obsidian-forge check-tags            [--vault <name>]  # отсутствующие теги layer/type/project
obsidian-forge check-tags --fix      [--vault <name>]  # добавить отсутствующие теги
obsidian-forge check-links           [--vault <name>]  # битые вики-ссылки (с учётом код-блоков)
obsidian-forge check-links --fix     [--vault <name>]  # исправить несоответствия имени файла/расширения
obsidian-forge normalize-frontmatter [--vault <name>]  # искажения YAML
obsidian-forge graph health          [--vault <name>]  # статистика и показатели здоровья
```

### Разовые операции

```bash
obsidian-forge sync               [--vault <name>]   # проверка здоровья графа → git
...
# [projects]
# exclude = ["_template"]           # дополнительные директории верхнего уровня, пропускаемые при сканировании
                                    # (директории, начинающиеся с точки, и node_modules всегда исключаются)
...
│   ├── init.rs        создание хранилища
│   ├── check_tags.rs  проверки здоровья тегов (--fix)
│   ├── check_links.rs проверки битых вики-ссылок (--fix)
│   ├── frontmatter.rs нормализация frontmatter (--fix)
│   ├── graph/
│   │   ├── wikilinks.rs извлечение и разрешение вики-ссылок
│   │   └── health.rs    отчёт о здоровье графа
...
- **obsidian-forge** = **Кузница** (запись/пуш). Фоновый демон, который автоматизирует обслуживание хранилища и синхронизирует с git.
...
- **[Velith](https://github.com/epicsagas/Velith)** = **Типография** (написание/публикация). Автономный ИИ-инструментарий для написания книг: черновик → редактура → публикация.
...
В то время как `obsidian-forge` фокусируется на поддержании механического здоровья вашего хранилища, [Alcove](https://github.com/epicsagas/alcove) гарантирует, что эти знания будут пригодны для использования ИИ-агентами для кодинга.

#### Как использовать их вместе:

1.  **Создавайте в Obsidian**: Используйте `obsidian-forge`, чтобы поддерживать хранилище в здоровом состоянии — маршрутизация входящих, проверки целостности, git-синхронизация.
