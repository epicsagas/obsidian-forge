...
obsidian-forge watch --vault <name> --interval <секунды>
```
...
- **obsidian-forge** = **Кузница (The Forge)** (запись/пуш). Фоновый демон, который автоматизирует обслуживание хранилища, укрепляет граф знаний и синхронизирует с git.
- **alcove** = **Библиотека (The Library)** (чтение/пулл). MCP-сервер, который предоставляет ИИ-агентам доступ к документации по запросу и с возможностью поиска, не раздувая окно контекста.
...
2.  **Повышайте статус до документов проекта**: Когда заметка (например, архитектурное решение или спецификация функции) готова для проекта, выполните команду `alcove promote --source путь/к/заметке.md`.
